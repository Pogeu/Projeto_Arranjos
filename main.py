...
def save_position_plot(positions: np.ndarray, title: str, path: Path) -> None:
    ...
    ax.scatter(positions[:, 0], positions[:, 1], positions[:, 2], s=34, c="#0b5cad", label="Sensores")
    ax.set_title(title)
...
def plot_upa(root: Path) -> list[dict[str, float | str]]:
    ...
    fig = plt.figure(figsize=(9.0, 4.0), constrained_layout=True)
    axes = [fig.add_subplot(1, 2, index, projection="3d") for index in (1, 2)]
    surfaces = []
    for ax, side in zip(axes, (3, 4), strict=True):
        positions = generate_upa(side, side, WAVELENGTH / 2.0, WAVELENGTH / 2.0)
    ...
    fig.colorbar(surfaces[-1], ax=axes, shrink=0.7, label="Ganho (dB)")
...
def write_generated_latex(root: Path, metrics_rows: list[dict[str, float | str]], link: dict[str, object]) -> None:
    selected_metrics = metrics_rows
    midpoint = (len(selected_metrics) + 1) // 2
    metric_pairs = zip(selected_metrics[:midpoint], selected_metrics[midpoint:], strict=True)
    lines: list[str] = [
    ...
    for left, right in metric_pairs:
        lines.append(
            f"{left['array']} & {left['config']} & "
            f"{format_float(float(left['beamwidth_deg']))} & "
            f"{format_float(float(left['sidelobe_db']))} & "
            f"{right['array']} & {right['config']} & "
            f"{format_float(float(right['beamwidth_deg']))} & "
            f"{format_float(float(right['sidelobe_db']))}\\\\"
        )
...
def run(root: Path, compile_article: bool = False) -> None:
    ...
    save_application_overview(root / "figures")
