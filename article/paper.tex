...
\section{Aplicacoes}
O processamento espacial de sinais aparece em diferentes sistemas praticos.
A Figura~\ref{fig:aplicacoes} resume as principais aplicacoes solicitadas no
trabalho.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{applications_overview.png}
\caption{Aplicacoes de processamento espacial em radar, sonar, comunicacoes
sem fio, sistemas acusticos e Massive MIMO.}
\label{fig:aplicacoes}
\end{figure}

\subsection{Radares}
Em radares, arranjos faseados permitem direcionar e varrer o feixe
eletronicamente, detectar alvos e formar nulos na direcao de interferencias
sem movimentar mecanicamente toda a antena \cite{matlab_radar}.

\subsection{Sonares}
Em sonares, arranjos de hidrofones estimam a direcao de fontes subaquaticas e
reforcam ecos provenientes de uma regiao de interesse. Arranjos maiores
produzem feixes mais estreitos e melhor separacao angular \cite{vantrees}.

\subsection{Comunicacoes sem fio}
Em comunicacoes sem fio, o beamforming concentra energia na direcao do usuario
e reduz interferencia para outros receptores. A combinacao de varios canais
tambem melhora a recepcao em ambientes com multipercurso
\cite{matlab_multichannel,tse}.

\subsection{Sistemas acusticos}
Matrizes de microfones sao usadas para captacao direcional, reconhecimento de
voz, videoconferencia e supressao de ruido. O mesmo vetor diretor empregado em
antenas pode ser adaptado para a velocidade de propagacao do som.

\subsection{Massive MIMO}
Massive MIMO utiliza dezenas ou centenas de antenas em uma estacao radio-base
para formar feixes independentes e atender varios usuarios simultaneamente,
aumentando o ganho espacial e a eficiencia espectral \cite{mpirical_mimo}.
